 ...
 */
router.post("/", authMiddleware, adminMiddleware, asyncHandler(async (req, res, next) => {
 ...
 * @access Private/Admin
 */
router.put("/:id", authMiddleware, adminMiddleware, asyncHandler(async (req, res, next) => {
  const { title, subtitle, image, linkURL, isActive, order } = req.body;
 ...
 * @access Private/Admin
 */
router.delete("/:id", authMiddleware, adminMiddleware, asyncHandler(async (req, res, next) => {
  const banner = await Banner.findById(req.params.id);
  if (!banner) {
    res.status(404);
    throw new Error("Banner not found");
